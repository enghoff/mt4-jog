...
| 19 | Cube yaw is irrelevant | Unmeasured — but "specific cubes repeatedly fail grasp at specific spots" (documented historically, reproduced attempt 8) matches this signature exactly | **MEDIUM** | Gripper closes at fixed orientation ("hold"); a cube sitting rotated ~45° is gripped on corners: unstable, off-center, and *stays* rotated across retries. Current retry logic can only route around it (session avoid-list), not fix it. A `minAreaRect` angle on the blob would make yaw observable cheaply. |
