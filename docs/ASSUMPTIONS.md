...
| 19 | Cube yaw is irrelevant | Unmeasured angle noise; `j4_face_offset_deg` (jaws vs j4=0) not yet fit on hardware | **MEDIUM** | Detection reports robot-frame edge yaw (`CubeDetection.yaw_deg`); `pick_cube` can face-align J4 via `pickplace.j4_for_face_align`, but `Calibration.face_align_picks` defaults **off** until the mount offset is validated (wrong offset → more corner grips than a fixed yaw). Enable in `vision_calibration.json` to test. |
